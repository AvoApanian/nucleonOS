...
runable/BIOS/driver/keyboard/keyboard.o:
	mkdir -p runable/BIOS/driver/keyboard/
	g++ $(CXXFLAGS) -c BIOS/driver/keyboard/keyboard.cpp -o runable/BIOS/driver/keyboard/keyboard.o


$(BUILD_DIR)/BIOS/kernel/kernel.elf: \
	$(BUILD_DIR)/BIOS/kernel/start.o \
	$(OBJ_FILES) \
	$(VGA_OBJ) \
	$(STRING_OBJ) \
	$(KEYBOARD_OBJ) \
	BIOS/kernel/linker.ld
...
$(BUILD_DIR)/BIOS/config.inc: \
	$(BUILD_DIR)/BIOS/stage2.bin \
	$(BUILD_DIR)/BIOS/kernel/kernel.bin
...
$(BUILD_DIR)/BIOS/boot.bin: \
	BIOS/bootloader/boot.asm \
	$(BUILD_DIR)/BIOS/config.inc
...
$(BUILD_DIR)/UEFIRun/BOOTX64.EFI: \
	$(BUILD_DIR)/UEFI/bootx64.o \
	UEFI/bootloader/linker.ls
...
# Image FAT32 reelle, remplie via mtools
